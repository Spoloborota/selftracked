...
`dod-shape-is-authoring-convention`) → rev 3.16 → the S8b open found
`gate-skip` events — instance-scoped like `paths`/`config`, but born only
at S8b, after the earlier carve-out — falling under R8 with no §4 entity to
resolve; the carve-out widens to include them (amendment
`gate-skip-joins-the-r8-carve-out`) → rev 3.17 → the 2026-07-24
open-questions round (five critics on the adjudicated fork analysis,
`docs/research/2026-07-24-open-questions-fork-analysis.md`) ratified four
wording-class amendments: a paused epic's IN-PROGRESS story stays in
`sprint_goals[]` by design (amendment `paused-epic-sprint-goal-is-intended`,
rev 3.18); the `load` prose matches the built `load` — no fast-forward
exists, `prime`'s flag owns the behind-DB case (amendment
`load-prose-matches-load`, rev 3.19); the pre-commit triages an incomplete
...
event date** (the one backfill door is `import`, §6.2: a row's date may
derive from cited commits or an explicit date field on any import,
`--legacy` alone additionally admits synthesized timestamps, and imported
rows are events-marked either way).
...
-- R8) — except the instance-scoped `paths`, `config` and `gate-skip`
-- events, whose entity carries the affected token verbatim (a dictionary
-- row or a configuration key has no §4 form; a gate skip has no entity at
-- all, so it carries a fixed instance token and records its moment in
-- detail); R8 skips those three by event type.
CREATE TABLE events (
...
if [ "$rc" = 1 ]; then
  ...
  echo "selftracked: verify did not complete (rc=$rc — interrupted or environment failure) — re-run; not bypassable as RED" >&2; exit 1
fi
...
is the no-`--force` form: it builds the DB when none exists and refuses any
existing one — a present-but-behind DB is `prime`'s case (the
`dump_divergence` flag), never `load`'s — so a refusal in the fallback chain
lands in the error branch by design. Stated limitation:
...
`sprint_goals[]` (every IN-PROGRESS story — **including one whose epic is
