...
Status: **DRAFT, revision 3.9 — for owner review.** Revision history: rev 1 →
...
this revision. Nothing here is code yet.
...
stated; the only nullable columns are `tasks.dup_of` and `tasks.epic`. Dates
...
characters refused at write time); NULL only in `tasks.dup_of`/`tasks.epic`
and `worklog.corrects`; no PRAGMAs ever.
