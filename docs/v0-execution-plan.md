...
Status: **ACTIVE, revision 7.** Revision history: rev 1 →
...
`review-proportionality-tiers` (D-EP7) → this revision. Governs the implementation of `docs/v0-spec.md`
...
|---|---|---|
| **S0** | Repo bootstrap: go.mod (toolchain pin §3.2), Makefile catalog, golangci config, CI skeleton (build/vet/test, `go fix -diff` both-checks, govulncheck), README skeleton, LICENSE/NOTICE intake | `make build lint test fix-check` exit 0; CI green Linux+macOS+Windows; the §16 `go fix -diff` exit-code re-verification runs here (a red-input probe confirming exit 1 on the pinned toolchain) |
...
resolving uncertainty upward (§5, D-EP7); (d) ledger
