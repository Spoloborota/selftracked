...
| S10 — dogfood switchover | FULL | in progress | — | Opened 2026-07-23 per D-EP13 (`docs/stage-openings/s10.md`): 1 row (INV-514), content/placement ok, no moves. Preconditions checked: the dup-chain poison pill fixed pre-stage; E3 (md-table bundled increments) carried as a corpus-authoring watch-item |
...
| The §9 pre-commit's rc-triage treats every non-{0,2} exit as a bypassable RED, including signal deaths (130/137/143) | S8b close review (shell-robustness critic) | **RESOLVED 2026-07-24** by amendment `rc-triage-verify-did-not-complete` (spec rev 3.20): RED is exactly rc=1; any other non-zero rc is "did not complete", not bypassable. |
| §11.1 says the fallback `load` "fast-forwards a missing/behind DB and refuses a divergent one"; the actual `load` refuses ANY existing DB without `--force` | S8c close review (spec-fidelity critic) | **RESOLVED 2026-07-24** by amendment `load-prose-matches-load` (spec rev 3.19): §8.4/§11.1 now describe the built `load`; the pipeline's divergence refusal names `load --force`. The chain was always correct — the prose now matches it. |
