...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (5, 'First push: re-run gates on the squashed tree and re-stamp every verified row (D-EP11)', 'OPEN', 'The publication boundary. Evidence recorded before the first push names commits a squash deletes; one re-run against the squashed tree re-stamps, and rows whose checks fail return to planned. The CI-half rows close there too: INV-492/499/513 (S0), INV-494/497/507 (S3), INV-498 (S4) - seven rows, per the retired ledger''s stage table (git history @ 0fffcbf). S12 privacy finding rides here: the squashed commit message must stay free of client-derived facts, including aggregate counts (a pre-squash message carried corpus row counts).', '', NULL, 'v0-bootstrap', '2026-07-23T22:09:23Z', '2026-07-24T01:39:46Z');
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (6, 'epic show --json emits a JSON header line plus plain-text sections, not one JSON document', 'OPEN', 'Found by the S10 close mechanics critic: epicShowFull appends stories/criteria/worklog as plain text regardless of --json (internal/verb/epics.go, S6-era commit cee9d82); spec says --json everywhere. Pre-S10 defect surfaced by the switchover''s cross-checks.', '', NULL, 'v0-bootstrap', '2026-07-23T22:28:38Z', '2026-07-23T22:28:38Z');
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (7, 'criteria check misdiagnoses unmet owner-attested criteria as ''a runnable criterion failed''', 'OPEN', 'Found at the S10 close: runCriteria (shared with epic close condition 3) counts unmet owner-attested criteria as blockers, and the standalone criteria check then refuses with ''a runnable criterion failed'' even when every runnable PASSed (internal/verb/criteria.go, close_conditions.go). The exit-1 may be intended; the message misnames the cause - the wrong-diagnosis class. S6-surface decision.', '', NULL, 'v0-bootstrap', '2026-07-23T22:29:35Z', '2026-07-23T22:29:35Z');
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (8, 'import: DUPLICATE with a later-positioned canonical refuses (forward dup_of)', 'NEEDS-TRIAGE', 'Rung-3 rehearsal finding: validateDupTarget runs before insert, so a corpus whose canonical task appears after its duplicate refuses with dup-target. Real corpora renumber canonicals late (a rename gives the canonical a higher id), so forward references are a natural shape. Either a two-pass dup_of resolution or a documented authoring constraint (canonical-before-duplicate, like E3 for md-table splits) - an S9-surface decision.', '', NULL, NULL, '2026-07-24T01:27:12Z', '2026-07-24T01:27:12Z');
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (9, 'import --legacy: an unresolvable cited commit is accepted silently; only R5 catches it', 'NEEDS-TRIAGE', 'Rung-3 rehearsal: a corpus worklog row citing a sha-shaped string that is not a commit of the repo imports without a warning (the git-first date engine falls back to synthesis) and surfaces only as a later verify R5 FAIL. A pre-flight import warning naming the unresolvable citation would move the finding to authoring time. S9-surface; observation, not a spec deviation (the two-step import-then-verify design is intentional).', '', NULL, NULL, '2026-07-24T01:28:05Z', '2026-07-24T01:28:05Z');
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (10, 'chaining recipe: a guarded chained line would survive directory-deletion abandonment', 'NEEDS-TRIAGE', 'S12 close review (docs critic): the recipe''s line ''.selftracked/hooks/pre-commit || exit $?'' breaks the host hook with exit 127 after rm -rf .selftracked - abandonment currently also requires removing the chained line (docs now say so). A guarded form ([ -x ... ] && ...) would make abandonment a pure directory deletion again. S8b-surface: the recipe text is pinned by fixtures and INV-421/422 wording, so the change travels as an amendment if taken.', '', NULL, NULL, '2026-07-24T01:49:31Z', '2026-07-24T01:49:31Z');
