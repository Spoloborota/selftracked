-- selftracked dump schema_version=1 tasks=25 artifacts=1
...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (25, 'Scaffolded divergence recipe names a verb form that always refuses', 'OPEN', 'SKILL.md and PROMPT.md (templates and instances) tell the agent to reconcile dump divergence with plain load - but plain load refuses whenever a DB exists (internal/load/verb.go: code=exists), and divergence implies an existing DB. The spec''s actual reconcile is load --force (replace local DB with the tracked dump, then re-apply unsynced local writes through verbs). Found by the S5 plan critic round, verified against source.', '', NULL, NULL, '2026-07-25T08:42:33Z', '2026-07-25T08:42:33Z');
