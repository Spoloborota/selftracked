-- selftracked dump schema_version=1 tasks=13 artifacts=0
...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (13, 'Harden the agent push barrier; decide template propagation and credential isolation', 'NEEDS-TRIAGE', 'Pre-publication hardening. Done this repo: .claude/settings.json deny-list now blocks Bash(git push:*) and bare git push at the Claude Code permission layer (parallel to the existing sqlite3 deny; enforced by the harness before the shell runs). Honest limits: a deny-list on a Bash command prefix raises the barrier for the ordinary invocation but is not airtight against a shell-capable agent (wrappers like ''git -c ... push'', cd into a submodule, eval, or a helper script can evade prefix matching). Two open decisions for the owner: (a) propagate the push-deny into the shipped scaffold template internal/scaffold/templates/claude_settings.json so every adopter inherits manual-push-only - this changes the product surface and needs an amendment; a matching pre-push git hook in the scaffold hook matrix (spec 8.4) would be defense-in-depth, also amendment-scope. (b) The only true wall is credential isolation: the agent''s shell must not possess push credentials (origin is https, so pushes currently authenticate via the machine credential helper the agent can reach). Recorded 2026-07-24.', '', NULL, NULL, '2026-07-24T13:26:03Z', '2026-07-24T13:26:03Z');
