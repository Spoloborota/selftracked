-- selftracked dump schema_version=1 tasks=49 artifacts=1
...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (48, 'Every epic close condition can be violated after the close', 'OPEN', 'The close gate is transition-time only, and nothing re-checks it. On a CLOSED epic these all succeed rc=0: story add (creates a PLANNED story - close condition 1), create --epic (homes an OPEN task - condition 4), criteria add (condition 3), edit --goal (rewrites the closed goal). No verify rule inspects a CLOSED epic, and there is deliberately no epic reopen (ddl.sql epics_transition comment), so a closed epic can permanently display the exact states its close gate exists to forbid. Spec §6.4 names ONE post-close surface - ''Post-close validation = V-n rows'' - and these are not it. Widens #35 from criteria to the whole condition set. Evidence: scratchpad drills, coordinator, 2026-07-25.', '', NULL, NULL, '2026-07-25T14:04:51Z', '2026-07-25T14:04:51Z');
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (49, 'criteria check rewrites a CLOSED epic''s acceptance record and destroys the close-time evidence', 'OPEN', 'criteria check re-executes runnables on ANY epic status and flips met 1->0 on failure (close_conditions.go runOne), overwriting the evidence column. Drilled: an epic closed with criterion ''$ test -f marker'' [met]; the file was later removed; one criteria check turned it to [open] and replaced the close-time PASS evidence with a FAIL line. The database and the events trail keep no copy - the trail records only ''criteria check: N line(s), failed=true''. CORRECTION (critic round, same day): the evidence is NOT unrecoverable in general - a committed dump.sql preserves the pre-overwrite value, so the loss is bounded by whether a commit captured the state first; the original claim ''exists nowhere else'' was too strong. The live-state record is still destroyed with no in-tracker copy, and there is no epic reopen. Also accepted on CLOSED: criteria met (rewrites evidence of a ratified criterion). Materiality is NOT zero here: this repository''s own CLOSED epic carries a runnable criterion, so one stray check while gates are red would overwrite its close-time evidence. Evidence: scratchpad drills + critic reproduction, 2026-07-25.', '', NULL, NULL, '2026-07-25T14:04:51Z', '2026-07-25T14:20:49Z');
