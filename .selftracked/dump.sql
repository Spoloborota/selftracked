...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (49, 'criteria check rewrites a CLOSED epic''s acceptance record and destroys the close-time evidence', 'OPEN', 'criteria check re-executes runnables on ANY epic status and flips met 1->0 on failure (close_conditions.go runOne), overwriting the evidence column. Drilled: an epic closed with criterion ''$ test -f marker'' [met]; the file was later removed; one criteria check turned it to [open] and replaced the close-time PASS evidence with a FAIL line. The events trail keeps only ''criteria check: N line(s), failed=true'' - the original evidence text exists nowhere else, so the acceptance record a CLOSED epic was ratified on is destructible with no audit copy (contradicting §1 principle 6, history is moved never deleted). Recovery exists only if the original condition can be re-satisfied; there is no epic reopen. Also accepted on CLOSED: criteria met (rewrites evidence of a ratified criterion). Evidence: scratchpad drills, coordinator, 2026-07-25.', '', NULL, NULL, '2026-07-25T14:04:51Z', '2026-07-25T14:04:51Z');
