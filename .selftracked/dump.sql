-- selftracked dump schema_version=1 tasks=57 artifacts=1
...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (40, 'md-table import: an unknown extra column is silently discarded', 'OPEN', 'md-table import: an unknown extra column was reported as silently discarded (S5 block G, filed 2026-07-25T09:12Z). RE-PROBED 2026-07-25 against the same binary and NOT REPRODUCIBLE. Four data-losing shapes were tried and every one is refused: an unknown header column ("tasks table has unknown column X", exit 1); a data row with more cells than the header ("has 4 cells but the header has 3", exit 1); a duplicate header column; and known-but-unused columns (pointer_note, owner_steer), whose data is folded into note rather than dropped. internal/verb/import_mdtable.go:242-243 checks every header cell against a closed per-kind vocabulary (knownColumns, line 264), and that check predates the filing - the file was last touched in acdd980 (2026-07-21), four days before this task was created. Either the original observation used a shape not captured in this note, or it is a misfiling. Needs an exact reproduction or a WONT-DO.', '', NULL, NULL, '2026-07-25T09:12:11Z', '2026-07-25T19:09:04Z');
...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (57, 'PO: does the INSERT-path posture still hold now that verb writes are guarded?', 'IN-REVIEW', 'PO: the spec section 1.1 accepts open INSERT paths on a detection-over-prevention argument, with verify as the detector. The terminal-epic amendment closed the verb path for the same class of damage, which leaves the two halves inconsistent: the same end state is refused when a verb asks for it and accepted when an import brings it. Three postures are coherent - guard the importer the same way, keep it open and treat R16 as the intended detector, or make R16 red rather than advisory for verb-closed epics. The choice is the owner s because it decides what a migration may do to history it has already closed. EVIDENCE ADDED 2026-07-25, and it narrows the question sharply: posture two is already the ratified position. openspec/changes/terminal-epic-conditions-stay-true/proposal.md:26 gives the unguarded import path as the reason R16 exists, and R16 scopes itself to verb-closed epics for exactly that case. So this was decided with open eyes, not overlooked, and reopening it needs an amendment that supersedes that one. What the amendment did NOT weigh: its stated ground for advisory-not-red is that "an imported history must never block a commit", but R16 never runs at the commit boundary - internal/verify/verify.go:118,146 place it in the full-only set and the generated hook calls verify --fast (internal/scaffold/templates/hooks/pre-commit:9). A red R16 would fail a full verify, that is a DoD gate, not a commit. The residual question is therefore only advisory versus red, on a ground the amendment did not actually test.', '', NULL, NULL, '2026-07-25T17:30:34Z', '2026-07-25T19:09:30Z');
