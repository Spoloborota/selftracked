-- selftracked dump schema_version=1 tasks=21 artifacts=1
...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (21, 'PROMPT.md: close the usability gaps a fresh-agent probe surfaced', 'OPEN', 'A fresh agent given only PROMPT.md hit four gaps: (1) the task-status vocabulary is never enumerated - set-status --help shows no valid target statuses, so the closing status had to be guessed; (2) the verb catalog lists names without signatures, forcing --help fallbacks for every call; (3) prime is not identified as the session-start read - its purpose is only inferable from the name; (4) the explicit-staging recipe for the bookkeeping commit (#19) is absent from PROMPT.md itself - it lives only in the scaffolded rule/skill files, so a PROMPT.md-only agent hits the empty-index refusal blind. Source: working-loop probe under #20.', '', NULL, NULL, '2026-07-25T08:01:28Z', '2026-07-25T08:01:28Z');
