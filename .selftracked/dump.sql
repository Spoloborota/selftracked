...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (53, 'import inserts children under an epic that a verb has closed', 'OPEN', 'The terminal-epic guards refuse every verb write that would reopen a close condition, but the importer is not guarded. A corpus whose epics list omits the epic and merely references its slug from a story or a task is accepted: exit 0, "imported 0 epic(s), 1 story(ies), 1 task(s)". verify then reports the result as advisory R16 lines - a non-terminal story and a workable task under a closed epic. A corpus that re-declares the epic is refused with code epic-exists, so the seam is exactly the children-only shape. This matters for any project that imports more than once - a delta import, a re-import after a corpus fix, an import into an instance that already holds closed epics - because the close conditions the guards protect can be reopened through the import path with no refusal, no warning at import time, and no record beyond the ordinary events row. Whether this is a defect or the deliberate INSERT-path posture is a spec question, raised separately.', '', NULL, NULL, '2026-07-25T17:30:34Z', '2026-07-25T17:30:34Z');
