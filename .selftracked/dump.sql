-- selftracked dump schema_version=1 tasks=35 artifacts=1
...
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (28, 'Refusal surface is split between domain messages and raw SQLite text', 'OPEN', 'Some refusals are clean domain envelopes (use-reopen, dup-chain, not-terminal); others leak raw CHECK/trigger text with SQLite error codes appended - park on a DONE task, DONE without note, LABEL transition all print constraint failed: CHECK constraint failed: ... (275)/(1811). Same command family, two message registers. S5 block B.', '', NULL, NULL, '2026-07-25T08:53:40Z', '2026-07-25T08:53:40Z');
INSERT INTO tasks (id, title, status, status_note, parked, dup_of, epic, created_at, updated_at) VALUES (29, '--json silently ignored by rel add/rm/tree/cycles and paths ls', 'OPEN', 'The spec promises --json on every verb; rel add/rm/tree/cycles and paths ls accept the flag and emit plain text anyway, and block D adds the epic family (epic list, epic show) - only list and verify were observed genuinely honoring --json on the success path (error envelopes are fine everywhere). Breaks scripted consumption of the documented contract. S5 blocks B, C, D.', '', NULL, NULL, '2026-07-25T08:53:40Z', '2026-07-25T09:01:16Z');
